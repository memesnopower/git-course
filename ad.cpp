...
int main(int argv, char *argc[]) {
    ...
    srand(key);
    gamma = rand();
    string source_string, decrypted_string, encrypted_string, result;
...
*/
        ...
        decryption(encrypted_string, decrypted_string);
        ofstream new1_file;
        new1_file.open(FILE_NAME);
        if (!new1_file) {
            cerr << "Can not open the file!";

            return 5;
        } else {
            new1_file << decrypted_string;
            new1_file.close();
        }
    }
...
/*
    if (MODE == "encryption") {
        ...
        ifstream new_file(FILE_NAME, ios::binary);
        if (!new_file) {
            cerr << "Can not open the file";
        ...
        string str;
        getline(new_file, str,'\0');
        new_file.close();
        decryption(encrypted_string, str);
        ofstream new1_file(FILE_NAME, ios::binary);
        if (!new1_file) {
            cerr << "Can not open the file";
